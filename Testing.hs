...
testFiltration = makeVRFiltrationFast [6.0, 5.0, 4.0] metric2 pointCloud
...
{--
  ...
  --}
  {--}
  putStrLn "Index bar codes of the point cloud:"
  putStrLn $ (intercalate "\n" $ L.map show $ L.map (L.filter (\(a, b) -> b /= Finite a)) $ indexBarCodesSimple testFiltration) L.++ "\n"
  --}
  {--
  putStrLn "Scale bar codes of the point cloud:"
  putStrLn $ (intercalate "\n" $ L.map show $ scaleBarCodesSimple [6.0, 5.0, 4.0] testFiltration) L.++ "\n"
  --}
  {--
  putStrLn "Non-simple filtration of the point cloud:"
  putStrLn $ filtr2String $ Right $ simple2Filtr $ testFiltration
  --}
  {--
  putStrLn "Index bar codes of the point cloud as a non-simple filtration:"
  putStrLn $ (intercalate "\n" $ L.map show $ indexBarCodes nsTestFiltration) L.++ "\n"
  --}
  {--
  putStrLn "Scale bar codes of the point cloud as a non-simple filtration:"
  putStrLn $ (intercalate "\n" $ L.map show $ scaleBarCodes [6.0, 5.0, 4.0] nsTestFiltration) L.++ "\n"
  --}
  {--
  putStrLn "Index bar codes of the octahedral cloud:"
  putStrLn $ (intercalate "\n" $ L.map show $ L.map (L.filter (\(a, b) -> b /= Just a)) $ indexBarCodesSimple octahedron) L.++ "\n"
  --}
  {--}
  putStrLn "Index bar codes of the square cloud:"
  putStrLn $ (intercalate "\n" $ L.map show $ L.map (L.filter (\(a, b) -> b /= Finite a)) $ indexBarCodesSimple square) L.++ "\n"
  --}
  {--}
  putStrLn "Index bar codes of Mobius strip:"
  putStrLn $ (intercalate "\n" $ L.map show $ L.map (L.filter (\(a, b) -> b /= Finite a)) $ indexBarCodesSimple mobiusStrip) L.++ "\n"
  --}